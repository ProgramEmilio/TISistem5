...
<body>
    ...
    <h1 class="titulo">Elminar Salones</h1>
    <?php
    include("../../../BD/ConexionBD.php");

    if (isset($_GET['Id_salon']) && !empty($_GET['Id_salon'])) {
        $Id_salon = mysqli_real_escape_string($conn, $_GET['Id_salon']);

        // Consulta para obtener la información del salón
        $sql = "SELECT salon.Id_salon, edificio.Id_edificio AS nom_edificio, tipo_salon.tipo AS tipo_salon, salon.descripcion 
                FROM salon
                JOIN edificio ON salon.Id_edificio = edificio.Id_edificio
                JOIN tipo_salon ON tipo_salon.id_ts = salon.tipo
                WHERE salon.Id_salon = '" . $Id_salon . "'";

        $result = $conn->query($sql);

        // Verifica si el salón existe
        if ($result->num_rows > 0) {
            $fila = $result->fetch_assoc();

            // Mostrar información del salón
            echo "<div class='info'>";
            echo "<p>ID del Salón: " . $fila['Id_salon'] . "</p>";
            echo "<p>Edificio: " . $fila['nom_edificio'] . "</p>";
            echo "<p>Tipo: " . $fila['tipo_salon'] . "</p>";
            echo "<p>Descripción: " . $fila['descripcion'] . "</p>";

            // Formulario para eliminar el salón
            echo "<form class='Eliminar_Salon' action='Eliminar_S.php' method='POST'>";
            echo "<input type='hidden' name='Id_salon' value='" . $fila['Id_salon'] . "'>";
            echo "<input type='submit' value='Eliminar Salón'>";
            echo "</form>";
            echo "</div>";
        } else {
            echo "<p>No se encontró el salón.</p>";
        }
    } else {
        echo "<p>No se proporcionó un ID de salón válido.</p>";
    }
...
?>
    ...
    <p><a href='../salon.php' class="regresar">Regresar</a></p>
</body>
